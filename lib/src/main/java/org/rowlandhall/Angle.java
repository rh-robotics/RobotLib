...
public class Angle {
    ...
    // relative to robot starting position (right = east, left = west, forward =
    // north, backward = south)
    public static final Angle RIGHT = new Angle(90, AngleType.NEG_180_TO_180_HEADING),
    ...
    public String toString() {
        return String.format("%s : %s", angle, type);
    }

    // assumes DEGREES for input and output!! use built-in Java method to convert
    // between radians and degrees
    // no other assumptions related to inputAngle value (can be -infinity to
    // infinity)
    ...
    public double convertAngleDouble(AngleType outputType) {
        ...
        if (sameNumericalSystem(type, outputType)) {
            ...
            // even though input and output types are not true to the type of intermediate
            // angle...
            // they have the correct important characteristic (numerical or coordinate)
            double angleNewNumericalSystem = convertNumericalSystem(angle, type,
                    numericalAndCoordinate(outputType, type)); // was type, output type
            double angleNewCoordinateSystem = convertCoordinateSystem(angleNewNumericalSystem,
                    numericalAndCoordinate(outputType, type), outputType); // was type, output type
    ...
    // example: direction FROM 0 degrees TO 90 degrees (both in
    // NEG_180_TO_180_HEADING type) is CLOCKWISE
    ...
    // INTERNAL METHODS - don't worry about these unless you're interested in how
    // this class works
    ...
    public static double convertCoordinateSystem(double inputAngle, AngleType inputType, AngleType outputType) {
        // ensure input and output coordinate system not same- assumed different later
        // on (bc of *-1)
        ...
        if (isCartesian(inputType)) {
            ...
            // *-1 or 360- is to flip direction (coordinate system change always causes
            // positive to flip between CW and CCW)
    ...
    // although this method currently is just a pass through, I think it may need to
    // do more in the future (and it adds uniformity)
    ...
    // returns an angle between max and min, assuming a coordinate system starting
    // at min and wrapping back to max
